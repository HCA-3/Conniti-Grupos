...
def auth_headers(role: str = "ADMIN", user_id: uuid.UUID | None = None) -> dict[str, str]:
    token = jwt.encode(
        {
...
def test_create_session_rejects_non_staff_role():
    response = client.post(
        "/",
        headers=auth_headers(role="PARTICIPANT"),
...
def test_staff_can_create_update_list_and_delete_session():
    ...
    delete_response = client.delete(f"/{session_id}", headers=auth_headers(role="SUPER_ADMIN"))
...
def test_registration_capacity_is_enforced_and_counter_is_authoritative():
    ...
    first = client.post(f"/{session_id}/register", headers=auth_headers("PARTICIPANT", first_user))
    full = client.post(f"/{session_id}/register", headers=auth_headers("PARTICIPANT", second_user))
    cancel = client.post(f"/{session_id}/register", headers=auth_headers("PARTICIPANT", first_user))
...
def test_static_config_route_precedes_uuid_route_and_uses_etag():
    ...
    stale = client.put(
        "/config", headers={**auth_headers("SUPER_ADMIN"), "If-Match": '"agenda-config-v0"'},
        json={
    ...
    updated = client.put(
        "/config", headers={**auth_headers("SUPER_ADMIN"), "If-Match": response.headers["etag"]},
        json={
            "edition_label": "CONIITI 2027",
            "conference_days": ["2026-10-01", "2026-10-02", "2026-10-03", "2027-09-10"],
            "timezone": "America/Bogota",
