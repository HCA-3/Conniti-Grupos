...
def test_signed_check_in_is_idempotent_one_use_and_eligible(monkeypatch):
    ...
    client.post(f"/{session['id']}/register", headers=auth_headers("PARTICIPANT", user_id))
    client.post(f"/{session['id']}/register", headers=auth_headers("PARTICIPANT", other_user_id))
    ...
    first = client.post(
        ...
        headers=auth_headers("PARTICIPANT", user_id), json={"token": token},
    )
    retry = client.post(
        f"/{session['id']}/attendance/check-in",
        headers=auth_headers("PARTICIPANT", user_id), json={"token": token},
    ...
    replay = client.post(
        f"/{session['id']}/attendance/check-in",
        headers=auth_headers("PARTICIPANT", other_user_id), json={"token": token},
    )
...
def test_manual_attendance_and_revocation_are_audited():
    ...
    revoked = client.patch(
        ...
        headers=auth_headers("ADMIN"), json={"reason": "Registro duplicado confirmado"},
    )
