...
def auth_headers(user_id: uuid.UUID, role: str = "PARTICIPANT") -> dict[str, str]:
    token = jwt.encode(
        {
            "sub": str(user_id),
            "type": "access",
...
def test_superuser_can_create_checkout_for_another_user():
    ...
    response = client.post(
        "/create-checkout",
        headers=auth_headers(current_user_id, role="SUPER_ADMIN"),
        json=payment_payload(other_user_id),
    )
...
def test_deployed_security_fails_closed_for_invalid_active_subject(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "post",
        lambda *_, **__: _IntrospectionResponse(
            {"active": True, "user_id": "not-a-uuid", "role": "SUPER_ADMIN"}
        ),
    )
