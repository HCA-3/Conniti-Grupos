...
async def get_current_user(request: Request) -> AuthenticatedUser:
    ...
    if normalized_role is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Auth devolvio un rol invalido.")
...
async def require_superuser(current: AuthenticatedUser = Depends(get_current_user)) -> AuthenticatedUser:
    if current.role not in {"SUPER_ADMIN", "ADMIN"}:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Se requiere rol administrativo.")
    return current
