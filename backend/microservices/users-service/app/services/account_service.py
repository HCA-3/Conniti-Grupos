...
def _ensure_superuser_continuity(user: User, changes: dict, db: Session) -> None:
    ...
    removes_active_superuser = (
        current_role == UserRole.SUPER_ADMIN.value
        and user.is_active
...
def update_profile_as_superuser(
    ...
    changes = payload.model_dump(exclude_unset=True)
    actor_role = actor.role.strip().upper()
    current_role = user.role.value if isinstance(user.role, UserRole) else str(user.role).upper()
    if actor_role == UserRole.ADMIN.value:
        if current_role != UserRole.PARTICIPANT.value:
            raise HTTPException(status_code=403, detail="Un administrador no puede modificar cuentas administrativas.")
        if "role" in changes and changes["role"] != UserRole.PARTICIPANT.value:
            raise HTTPException(status_code=403, detail="Solo el superadministrador puede asignar roles administrativos.")
    ...
    role_changed = new_role is not None and new_role != current_role
