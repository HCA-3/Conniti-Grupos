...
def _is_admin(actor: AuthenticatedUser) -> bool:
    return _global_role(actor).upper() in {UserRole.SUPER_ADMIN.value, UserRole.ADMIN.value}
...
def update_group(
    ...
    if not _is_admin(actor) and "is_active" in changes:
...
def add_member(
    ...
    if not _is_admin(actor) and payload.membership_role == GroupMembershipRole.GROUP_ADMIN:
...
def update_member(
    ...
    if not _is_admin(actor) and ("membership_role" in changes or target_is_admin):
...
def remove_member(group_id: str, user_id: str, actor: AuthenticatedUser, db: Session) -> None:
    ...
    if not membership:
        raise HTTPException(status_code=404, detail="Membresia no encontrada.")
    target_is_admin = _membership_role(membership) == GroupMembershipRole.GROUP_ADMIN.value
    if not _is_admin(actor) and target_is_admin:
