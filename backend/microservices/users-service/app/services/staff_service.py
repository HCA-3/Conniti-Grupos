...
def create_staff_account(user: StaffCreateRequest, db: Session):
    if user.role != UserRole.ADMIN.value:
        raise HTTPException(status_code=422, detail="Solo se permiten cuentas de administrador.")
...
def update_staff_account(user_id: str, user_data: StaffUpdateRequest, db: Session):
    ...
    if _role_value(user) != UserRole.ADMIN.value:
        raise HTTPException(status_code=404, detail="Cuenta de administrador no encontrada.")
    if user_data.role is not None and user_data.role != UserRole.ADMIN.value:
        raise HTTPException(status_code=422, detail="Este endpoint no permite cambiar el rol administrativo.")
