...
def auth_headers(user_id: str, role: str = "PARTICIPANT") -> dict[str, str]:
    token = jwt.encode(
        {
            "sub": user_id,
            "type": "access",
...
def test_user_can_complete_optional_profile_after_registration(monkeypatch):
    ...
    seed_user(user_id, "PARTICIPANT", full_name="OAuth Person", email="oauth@example.com")
...
def test_admin_profiles_are_paginated_and_last_superuser_is_protected():
    admin_id = "admin-one"
    seed_user(admin_id, "SUPER_ADMIN", full_name="Admin", email="admin@example.com")
    seed_user("user-two", "PARTICIPANT", full_name="Second", email="second@example.com")

    listing = client.get(
        "/admin/profiles?page=1&page_size=1&search=second",
        headers=auth_headers(admin_id, "SUPER_ADMIN"),
    )
...
def test_role_change_suspends_auth_before_commit_and_reactivates_with_new_version(monkeypatch):
    ...
    seed_user(actor_id, "SUPER_ADMIN", full_name="Actor", email="actor@example.com")
    seed_user(target_id, "SUPER_ADMIN", full_name="Target", email="target@example.com")
    calls = []
    monkeypatch.setattr(
        auth_client,
    ...
    response = client.patch(
        ...
        headers=auth_headers(actor_id, "SUPER_ADMIN"),
...
def test_internal_profile_summaries_return_no_extra_pii():
    seed_user("winner", "PARTICIPANT", full_name="Winner Name", email="private@example.com")
...
def test_group_admin_is_scoped_and_cannot_grant_group_admin_role():
    ...
    seed_user(super_id, "SUPER_ADMIN", full_name="Super", email="super@example.com")
    seed_user(group_admin_id, "PARTICIPANT", full_name="Group Admin", email="ga@example.com")
    seed_user(member_id, "PARTICIPANT", full_name="Member", email="member@example.com")
    seed_user(outsider_id, "PARTICIPANT", full_name="Outsider", email="out@example.com")

    created = client.post(
        "/groups",
        headers=auth_headers(super_id, "SUPER_ADMIN"),
