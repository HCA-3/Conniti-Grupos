...
def _normalized_role(profile: dict) -> str:
    raw_role = str(profile.get("role", "PARTICIPANT")).strip()
    aliases = {
        ...
        "viewer": "PARTICIPANT",
        "docente": "PARTICIPANT",
        "estudiante": "PARTICIPANT",
        "externo": "PARTICIPANT",
        "external": "PARTICIPANT",
        "university_community": "PARTICIPANT",
        "user": "PARTICIPANT",
    }
    normalized = aliases.get(raw_role.lower(), raw_role.upper())
    if normalized not in {"SUPER_ADMIN", "ADMIN", "PARTICIPANT"}:
        raise HTTPException(status_code=503, detail="Users devolvio un rol invalido.")
    return normalized
