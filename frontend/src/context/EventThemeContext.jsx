...
function applySiteCustomVariables(config) {
    if (typeof document === 'undefined') return;
    const root = document.documentElement;
    const merged = { ...SITE_CUSTOM_DEFAULTS, ...(config ?? {}) };
...
function loadSiteConfigFromStorage() {
    try {
