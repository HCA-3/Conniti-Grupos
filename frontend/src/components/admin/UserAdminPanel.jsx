...
// Roles del sistema — mapeo a etiqueta en español.
const ROLE_LABELS = {
...
export default function UserAdminPanel() {
    ...
    const canManageAdmins = actor?.role === 'SUPER_ADMIN';
    ...
    return (
        <section className={styles.section}>
                            ...
                            <tbody>
                                {result.items.map((user) => (
                                    <tr key={user.id} className={user.is_active ? '' : styles.inactive}>
                                        ...
                                        <td><button className={styles.smallButton} type="button" disabled={!canManageAdmins && user.role !== 'PARTICIPANT'} onClick={() => setEditing(user)}><FiEdit2 /> Editar</button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            {editing && <EditProfileModal user={editing} canManageRoles={canManageAdmins} onClose={() => setEditing(null)} onSaved={handleSaved} />}
        </section>
    );
}


function EditProfileModal({ user, canManageRoles, onClose, onSaved }) {
        ...
        <div className={styles.overlay} role="presentation" onMouseDown={onClose}>
            <form className={styles.modal} onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <div className={styles.grid}>
                    ...
                    <div className={styles.field}><label htmlFor="admin-role">Rol</label><select id="admin-role" name="role" value={form.role} onChange={change} disabled={!canManageRoles}>{Object.entries(ROLE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
